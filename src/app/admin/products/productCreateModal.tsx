...
interface ProductFormModalProps {
    ...
    onApplyDiscount: (dto: UpdateAdminProductDiscount) => void;
    onClose: () => void;
    onDisableDiscount: () => void;
...
export default function ProductFormModal({
    ...
    const [discountMode, setDiscountMode] = useState<"price" | "percentage">(
        "price",
    );
    ...
    const hasDiscount = Boolean(product?.hasDiscount && product.discountPrice != null);
    const isDiscountDisabled = isSubmitting || isUpdatingDiscount;
    ...
    }, [product?.id, product?.hasDiscount, product?.discountPrice]);
    ...
    function applyDiscount() {
        ...
        if (discountMode === "price") {
            const nextDiscountPrice = Number(discountPrice);

            if (
                !Number.isFinite(nextDiscountPrice) ||
                nextDiscountPrice <= 0 ||
                nextDiscountPrice >= product.price
            ) {
                setDiscountFormError(
                    "Enter a discount price greater than 0 and lower than the original price.",
                );
                return;
            }
        ...
        const nextDiscountPercentage = Number(discountPercentage);

        if (
            !Number.isFinite(nextDiscountPercentage) ||
            nextDiscountPercentage < 1 ||
            nextDiscountPercentage > 99
        ) {
            setDiscountFormError(
    ...
    function handleDiscountKeyDown(event: React.KeyboardEvent<HTMLDivElement>) {
        if (event.key !== "Enter") {
            return;
        }
    ...
    return (
        <div
            ...
            onClick={isDiscountDisabled ? undefined : onClose}
        >
            <section
            ...
            >
                <header className="flex items-start justify-between gap-4 border-b border-zinc-200 px-5 py-5 sm:px-6">
                    ...
                    <button
                        ...
                        disabled={isSubmitting || isUpdatingDiscount}
                        onClick={onClose}
                        type="button"
                    >
                    ...
                    <div className="space-y-5 px-5 py-6 sm:px-6">
                        ...
                        {isEditing && product && (
                            <div className="rounded-lg border border-zinc-200 p-4">
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        ...
                                        {hasDiscount ? (
                                            ...
                                            <p className="mt-1 text-sm text-zinc-500">
                                                This product does not have a discount yet.
                                            </p>
                                        )}
                                    </div>
                                    {hasDiscount ? (
                                        <button
                                            className="inline-flex h-10 cursor-pointer items-center justify-center rounded-md border border-red-200 px-4 text-sm font-semibold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
                                            disabled={isDiscountDisabled}
                                            onClick={onDisableDiscount}
                                            type="button"
                                        >
                                            {isUpdatingDiscount
                                                ? "Disabling..."
                                        ...
                                        <button
                                            className="inline-flex h-10 cursor-pointer items-center justify-center rounded-md border border-zinc-300 px-4 text-sm font-semibold text-zinc-800 transition hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-60"
                                            disabled={isDiscountDisabled}
                                            onClick={() =>
                                                setIsDiscountFormOpen((current) => !current)
                                            }
                                            type="button"
                                        >
                                            Apply discount
                                ...
                                {!hasDiscount && isDiscountFormOpen && (
                                    <div
                                        className="mt-4 rounded-md bg-zinc-50 p-3"
                                    ...
                                    >
                                        ...
                                        {discountMode === "price" ? (
                                            <label className="mt-3 block">
                                                ...
                                                <input
                                                    className="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm outline-none transition focus:border-zinc-950 focus:ring-2 focus:ring-zinc-200"
                                                    max={Math.max(product.price - 0.01, 0)}
                                                    min="0.01"
                                                    onChange={(event) =>
                                                        setDiscountPrice(event.target.value)
                                                    }
                                                    placeholder={`Less than ${formatPrice(product.price)}`}
                                                    step="0.01"
                                                    type="number"
                                                    value={discountPrice}
                                                />
                                                <p className="mt-1 text-xs text-zinc-500">
                                                    The discount price must be lower than the original price.
                                                </p>
                                            </label>
                                        ) : (
                                            <label className="mt-3 block">
                                        ...
                                        <button
                                            className="mt-3 inline-flex h-10 cursor-pointer items-center justify-center rounded-md bg-zinc-950 px-4 text-sm font-semibold text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-60"
                                            disabled={isDiscountDisabled}
